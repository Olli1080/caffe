...
void HDF5OutputLayer<Dtype>::LayerSetUp(const std::vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void HDF5OutputLayer<Dtype>::Forward_cpu(const std::vector<Blob<Dtype>*>& bottom,
      const std::vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void HDF5OutputLayer<Dtype>::Backward_cpu(const std::vector<Blob<Dtype>*>& top,
...
#else
template <typename Dtype>
void HDF5OutputLayer<Dtype>::Forward_gpu(const std::vector<Blob<Dtype>*>& bottom,
    const std::vector<Blob<Dtype>*>& top) {
    CHECK_GE(bottom.size(), 2);
    CHECK_EQ(bottom[0]->num(), bottom[1]->num());
    data_blob_.Reshape(bottom[0]->num(), bottom[0]->channels(),
...
template <typename Dtype>
void HDF5OutputLayer<Dtype>::Backward_gpu(const std::vector<Blob<Dtype>*>& top,
    const std::vector<bool>& propagate_down, const std::vector<Blob<Dtype>*>& bottom) {
    return;
}
#endif
