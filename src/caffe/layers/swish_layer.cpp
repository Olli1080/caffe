...
void SwishLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    ...
    if (propagate_down[0]) {
        ...
        backward_kernel(count, top_diff, top_data, sigmoid_output_data, bottom_diff, beta);
        CUDA_POST_KERNEL_CHECK;
    }
}

extern template void SwishLayer<float>::backward_kernel(const int, const float*, const float*, const float*, float*, const float);
extern template void SwishLayer<double>::backward_kernel(const int, const double*, const double*, const double*, double*, const double);
#endif
