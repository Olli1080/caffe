...
void TileLayer<Dtype>::Forward_cpu(
  ...
  for (unsigned int i = 0; i < outer_dim_; ++i) {
    for (unsigned int t = 0; t < tiles_; ++t) {
...
template <typename Dtype>
void TileLayer<Dtype>::Backward_cpu(const std::vector<Blob<Dtype>*>& top,
  ...
  for (unsigned int i = 0; i < outer_dim_; ++i) {
    ...
    top_diff += inner_dim_;
    for (unsigned int t = 1; t < tiles_; ++t) {
...
STUB_GPU(TileLayer);
#else
INSTANTIATE_LAYER_GPU_FUNCS_EXTERN(TileLayer);
#endif
